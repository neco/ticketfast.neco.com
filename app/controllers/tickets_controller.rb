...
class TicketsController < ApplicationController
  ...
  def list
    ...
    unless params[:show_all] || params[:viewed_only]
    ...
    ticket_json = @tickets.to_json(
      :only => [:id, :event_id, :section, :row, :seat, :email_sent_at, :email_from, :email_subject],
      :include => {
        :event => {
          :only => [:name, :venue_id, :occurs_at, :code],
          :include => {
            :venue => { :only => :name }
          }
        }
    ...
    render :text => %[{"totalRecords":#{Ticket.count(:include => find_include, :conditions => find_conditions)},
  ...
  def get_event_dates
    conditions = (params[:show_all].empty? && params[:viewed_only].empty?)  ? ['occurs_at > ?', 1.day.ago] : []
  ...
  def event
    ...
    @tickets = @event.tickets.all(:conditions => ['viewed = ?', false])
    @viewed_tickets = @event.tickets.all(:conditions => ['viewed = ?', true])
    render :action => "index"
  end
  ...
  def view_queue
    ...
    #@tickets = Ticket.unparsed.all(:order => 'created_at DESC', :limit => 50)
  end

  def view_text
    @ticket = Ticket.find params[:id]
    out_path = "#{Rails.root}/tmp/#{@ticket.id}_text"
  ...
  def queue_set_event
    ...
    if params[:event_id] && (params[:custom_date].nil? || params[:custom_date][ticket.id.to_s].empty?)
  ...
  def email_or_download_tickets
    if !params[:tickets] || params[:tickets].size == 0
      redirect_to :action => "index"
      return
    end
    ...
    ticket_actions.each { |ticket_action| ticket_action.save }
  end
