class TmAccountsController < ApplicationController
  ...
  def auto_complete_for_ticket_tm_event_name
    ...
    @items = Ticket.unfetched.all(find_options)
  ...
  def dt_unfetched
    ...
    if params[:tm_event_name] && !params[:tm_event_name].blank?
      find_conditions[0] += ' AND tm_event_name = ?'
      find_conditions << params[:tm_event_name].strip
    end

    if params[:tm_venue_name] && !params[:tm_venue_name].blank?
    ...
    render :text => %[{"totalRecords":#{Ticket.unfetched.count(:include => find_include, :conditions => find_conditions)},
  ...
  def list
    ...
    render :text => %[{"totalRecords":#{TmAccount.count(:include => find_include, :conditions => find_conditions)},
