class FetcherJob < ActiveRecord::Base
  ...
  def self.fetch_request args
    ...
    if old_job && old_job.job_status != status(:open)
      old_job.destroy
    end
  ...
  def self.register_client_done(client_key)
    all(:conditions => { :client_key => client_key }).each { |job| job.destroy }
  end
  ...
  # Should use job key instead of client key, but doesn't matter right now
  def self.submit_work(args)
    client_key, results, remote_ip = args[:client_key], args[:results], args[:remote_ip]
    found_job = find_by_client_key_and_job_target_and_job_status(client_key, remote_ip, status(:working))
    found_job.update_attributes(:job_results => results, :job_status => status(:ready)) if found_job
  end

  def self.fetch_response(args)
    job_key = args[:job_key]
    job = first(:conditions => {:job_key => job_key, :job_status => status(:ready)})
